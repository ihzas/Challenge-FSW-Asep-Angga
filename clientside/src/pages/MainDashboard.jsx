...
export default function MainDashboard() {
  ...
  useEffect(() => {
    const fetchCars = async () => {
      try {
        const response = await axios.get("http://localhost:4000/cars/cars");
  ...
  const handleDelete = (deletedCarId) => {
        ...
        <div className="flex flex-col md:pl-64">
          ...
          <main className="flex-row">
            <div className="py-6">
              ...
              <div className="mx-auto max-w-7xl w-full px-4 sm:px-6 md:px-8 flex flex-wrap gap-4">
                {cars.map((car) => (
                  <CarCard
                    key={car.id}
                    car={car}
